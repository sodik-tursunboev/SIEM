...
# Flask defaults to looking for templates/ and static/ next to this file.
# Inside a PyInstaller bundle those folders are unpacked into a temp
# directory instead, so the paths must be resolved explicitly or every
# page returns TemplateNotFound and every stylesheet 404s. Running from
# source this resolves to exactly the same folders as before.
app = Flask(
    __name__,
...
app.register_blueprint(bas.bp)

# The ingest endpoint accepts events from remote forwarders. In the
# public demo it is NOT registered at all - not merely locked down.
# Reasoning: its only auth is a shared secret in a plaintext header, and
# a publicly reachable /api/ingest/event is an open invitation to inject
# fabricated alerts into the demo everyone is looking at. Removing the
# route entirely means there is no endpoint to attack, no key to leak,
# and no rate limit to tune. Local/lab runs register it as normal.
...
_SECRET_PATH = paths.data_path("instance_secret.key")
...
# ---------------------------------------------------------------------
# HTTPONLY blocks JavaScript from reading the session cookie, so an XSS
# bug can't be escalated into session theft. SAMESITE=Lax stops the
# cookie riding along on cross-site POSTs (CSRF). SECURE is read from an
# env var rather than hardcoded True, because this lab runs over plain
# HTTP - setting it True here would silently break login locally. Flip
# SIEM_HTTPS=1 the moment this sits behind TLS.
app.config.update(
    SESSION_COOKIE_HTTPONLY=True,
    SESSION_COOKIE_SAMESITE="Lax",
    SESSION_COOKIE_SECURE=os.environ.get("SIEM_HTTPS") == "1",
...
def _security_headers(resp):
    """Defence-in-depth response headers applied to every route."""
    resp.headers["X-Content-Type-Options"] = "nosniff"
    resp.headers["X-Frame-Options"] = "DENY"
    resp.headers["Referrer-Policy"] = "no-referrer"
    resp.headers["Permissions-Policy"] = "geolocation=(), microphone=(), camera=()"
    # KNOWN LIMITATION: 'unsafe-inline' is present on script-src because
    # all 18 templates carry inline <script> blocks - removing it blocks
    # every page's JavaScript. That weakens CSP's XSS containment, so it
    # is deliberate and documented rather than quietly ignored. Proper fix
    # is per-request nonces (see README hardening notes). The other
    # directives still hold: no external script origins, no framing, no
    # object/embed, base-uri locked.
    ...
    if os.environ.get("SIEM_HTTPS") == "1":
...
def login():
    if request.method == "POST":
        ...
        # Per-IP check runs FIRST, before the username lockout. Order
        # matters: a spraying attacker cycles usernames precisely so no
        # single account ever locks, so checking the username first would
        # let every attempt through. Checking the source address first
        # catches the pattern the username lockout is blind to.
        if client_ip and db.is_ip_rate_limited(client_ip):
            db.record_login_attempt(username, client_ip, False)
            collector.save_event(
                source="SIEM", event_id=rules.EVENT_SIEM_LOGIN_FAILED,
                event_type="SIEM_LoginRateLimited", user=username, source_ip=client_ip,
                host="mini-siem",
                message=f"Login attempt from {client_ip} rejected - source address "
                        f"rate limited after repeated failures across accounts. "
                        f"Consistent with password spraying.",
            )
            flash("Too many failed attempts from this address. Try again in a few minutes.", "error")
