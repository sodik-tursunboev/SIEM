...
# Scoped to this process only - resets the moment this window closes, no
# permanent system change. Needed because Import-Module below (and some
# atomic tests themselves) fail under the default Restricted policy that
# a plain PowerShell window opens with.
...
# Adjust these two paths if your install location differs.
try {
    Import-Module "C:\Tools\invoke-atomicredteam-master\Invoke-AtomicRedTeam.psd1" -Force -ErrorAction Stop
    $PSDefaultParameterValues["Invoke-AtomicTest:PathToAtomicsFolder"] = "C:\Tools\atomic-red-team-master\atomics"
} catch {
    Write-Host "[ERROR] Could not import Invoke-AtomicRedTeam - check the path above matches where you extracted it: $_" -ForegroundColor Red
    exit 1
}
...
# ---------------------------------------------------------------------
$TestPlan = @(
    @{ TechniqueId = "T1003.001"; TechniqueName = "OS Credential Dumping: LSASS Memory";        FriendlyName = "Dump LSASS.exe Memory using comsvcs.dll"; TestNumbers = "2" }
...
foreach ($test in $TestPlan) {
    ...
    try {
        ...
        if ($test.TestNumbers) {
            Invoke-AtomicTest $test.TechniqueId -TestNumbers $test.TestNumbers -GetPrereqs -ErrorAction SilentlyContinue
            Invoke-AtomicTest $test.TechniqueId -TestNumbers $test.TestNumbers
        } else {
            Write-Host "  [NOTE] Running ALL sub-tests for this technique - some may fail if they need tools this VM can't download (no internet). That's expected, not a SIEM bug. If this technique ends up Missed, check the console output above for which sub-test actually succeeded (Exit code: 0), then add a TestNumbers value for it in `$TestPlan." -ForegroundColor DarkGray
    ...
    try {
        if ($test.TestNumbers) {
            Invoke-AtomicTest $test.TechniqueId -TestNumbers $test.TestNumbers -Cleanup -ErrorAction SilentlyContinue
        } else {
            Invoke-AtomicTest $test.TechniqueId -Cleanup -ErrorAction SilentlyContinue
        }
    } catch {}
