...
The question every detection rule eventually has to answer isn't "did I write this rule" — it's "does it actually catch the thing it's supposed to catch, in time." Sentinel BAS answers that with a number instead of a guess.

It drives [Atomic Red Team](https://github.com/redcanaryco/atomic-red-team) against the live lab, records exactly when each technique started and ended, and automatically checks whether a matching alert appeared — matched by MITRE technique (hierarchy-aware: a test for `T1003` correctly credits an alert tagged `T1003.001`), scoped to the same host, and scored by timing: within 60 seconds is **Detected**, within 5 minutes is **Delayed**, nothing by then is **Missed**.

**Real result from a full run against the live lab: 70% coverage — 7 Detected, 0 Delayed, 3 Missed, 0 stuck Pending.**

A Missed result isn't automatically a detection gap. There are three distinct reasons a technique can miss, and they mean different things:

1. **The attack never actually ran** (missing external tooling, no internet in an isolated lab) — not a detection gap, just an environmental limitation of the simulation itself
2. **The attack ran but didn't reproduce the exact condition the rule targets** — for example, the Kerberoasting Atomic test negotiated modern AES encryption against built-in system SPNs (after LDAP enumeration failed and it fell back to defaults) rather than the weak RC4 condition against custom service accounts that real Kerberoasting — and this rule — specifically target. A legitimate finding about the simulation, not the rule.
3. **The attack ran, matched the rule's conditions, and still nothing fired** — a genuine gap worth fixing

All 3 misses in this run were reasons 1 and 2, not 3.

Building this also surfaced and fixed three separate, previously-undiscovered bugs in the log pipeline: a Windows event field (`TicketEncryptionType`) that was never being extracted from Kerberos ticket events at all, meaning the Kerberoasting rule was structurally unable to fire regardless of what actually happened on the wire; and a self-noise problem where the forwarder was faithfully logging its own polling process and its own HTTP calls to the SIEM, flooding real telemetry with thousands of self-referential log entries.

`bas_runner.ps1` runs the full technique plan end to end and reports live status; `/bas` shows the running coverage dashboard.
